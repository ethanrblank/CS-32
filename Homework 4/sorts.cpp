...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Student*> auxStudentsPointers;
        for (int i = 0; i < auxStudents.size(); ++i) {
            auxStudentsPointers.push_back(&auxStudents[i]);
        }
        
        sort(auxStudentsPointers.begin(),auxStudentsPointers.end(), compareStudentPtr);
        
        for (int i = 0; i < auxStudentsPointers.size(); ++i) {
            students[i] = *(auxStudentsPointers[i]);
        }
