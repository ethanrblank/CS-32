...
{
    if (n <= 1) // if the array does not "exist"
        return;
    int before; // variables for element that should come before the current
    int after; // variable for element that should come after the current
    
    separate(a, n, a[0], before, after);
    order(a, before);
